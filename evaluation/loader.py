...
def load_data_session_retrain_nextitnet(path, file, trian_set, test_idx, sessions_train=None, sessions_test=None, slice_num=None, train_eval=False):
    '''
    Loads a tuple of training and test set with the given parameters.
    ...
    trian_set: int
        ...
        train-item-views_train_full.0.txt
    test_idx: int
        The index of the test day in test data. Adds another postfix to the postfix of the train set for test data
        for ex test_idx: 7 will create train-item-views_test.0_7.txt
    slice_num :
        Adds a slice index to the constructed file_path
        yoochoose-clicks-full_train_full.0.txt
    train_eval : boolean
    ...
    test_list = []
    # for n in range(0,test_num):
    split = '.' + str(trian_set) + '_' + str(test_idx)
    test = pd.read_csv(path + file + test_appendix + split + '.txt', sep='\t', dtype={'ItemId': np.int64})
    test_list.append(test)
