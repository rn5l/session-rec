        ...
        for j in range( self.epochs ):
            ...
            while not finished:
                ...
                if self.samples > 0:
                    ...
                    loss += self.train_model_batch( sci, np.hstack( [ipos, additional] ) )
                else:
                    loss += self.train_model_batch( sci, ipos )
...
#                 hit += ( (preds.T > val_pos).sum(axis=0) < 20 ).sum()
    ...
    def _generate_train_model_batch_function(self):
        ...
        updates = getattr(self, self.learn)(cost, [self.I1,self.I2,self.BI,self.H], self.learning_rate)
        updates.append( (self.h_t, h_t_new) )
        
        self.train_model_batch = theano.function(inputs=[i, y], outputs=cost, updates=updates )
    ...
    def _generate_predict_function(self):
        ...
        self.predict = theano.function(inputs=[i], outputs=pred, updates=[(self.h_t_p, h_t_p_new)] )
    ...
    def _generate_predict_batch_function(self):
        ...
        self.predict_batch = theano.function(inputs=[i], outputs=pred ) #, updates=updates )
    ...
    def predict_next( self, session_id, input_item_id, predict_for_item_ids, skip=False, type='view', timestamp=0 ):
        ...
        predictions = self.predict( self.item_map[input_item_id] )
