...
class SessionMF:
    ...
    def __init__( self, factors=100, batch=50, learn='adagrad', learning_rate=0.001, regularization=0.0001, dropout=0, skip=0, samples=2048, activation='relu', objective='bpr', epochs=5, last_n_days=None, session_key = 'SessionId', item_key= 'ItemId', time_key= 'Time' ):
        ...
        for j in range( self.epochs ):
            ...
            for row in train.itertuples(index=False):
                ...
                if len(session_list) > 0: 
                    spi[ batch_count ] = item
                    sci[ batch_count ] = session_list[-1]
                    svec[ sci[ batch_count ] ] = len( session_list )
                    smat[ batch_count ] = svec / len( session_list )
                    batch_count += 1
#                     if len(items) == 0:
#                         items.extend( session_list )
#                     else:
#                         items.append( item )
                    items.update( session_list )
                ...
                if batch_count == self.batch:
                    ...
                    if self.samples > 0:
                        ...
                        #stmp = time.time()
                        loss += self.train_model_batch( smat, sci, np.hstack( [spi, additional] ), list(items) )
                        #avg_time += (time.time() - stmp)
                        #avg_count += 1
                    else:
                        loss += self.train_model_batch( smat, sci, spi, items )
...
#                     print( len(sitems) )
    ...
    def _generate_train_model_batch_function(self):
        ...
        updates = getattr(self, self.learn)(cost, [self.S,self.I,self.I1,self.I2,self.BI,self.BS], [Sit,Iy,I1i,I2y,BIy,BSy],[items,y,i,y,y,y], self.learning_rate)
