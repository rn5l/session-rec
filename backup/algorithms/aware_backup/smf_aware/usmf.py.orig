        ...
        for j in range( self.epochs ):
            ...
            if j <= self.shuffle:
                for i in self.sessions: 
                    random.shuffle(self.session_map[i])
                print( 'shuffled sessions for epoch', j )
            else:
            ...
            scu = np.zeros( self.batch ).astype( self.intX )
            ...
            while not finished:
                ...
                for i in range(self.batch):
                    ...
                    user_current = self.session_user[ self.sessions[ sidx[i] ] ]
                ...
                if self.samples > 0:
                    ...
                    if itemsl is None:
                        itemsl = list(items)
                    loss += self.train_model_batch( smat, sci, scu, np.hstack( [ipos, additional] ), itemsl )
                    avg_time += (time.time() - stmp)
                    avg_count += 1
                else:
                    loss +=  self.train_model_batch( smat, sci, scu, ipos, scp )
...
#                 hit += ( (preds.T > val_pos).sum(axis=0) < 20 ).sum()
    ...
    def init_model(self, train, std=0.01):
        ...
        self.U = theano.shared( np.random.normal(0, std, size=(self.num_users, self.factors) ).astype( self.floatX ), name='U' )
        ...
        self.BU = theano.shared( np.random.normal(0, std, size=(self.num_items,1) ).astype( self.floatX ), name='BS' )
    ...
    def predict_next( self, session_id, input_item_id, predict_for_item_ids, input_user_id=None, skip=False, type='view', timestamp=0 ):
        ...
        predictions = self.predict( self.session_items / self.session_count, self.item_map[input_item_id], self.user_map[input_user_id] )
