...
class SessionMF:
    ...
    def __init__( self, factors=100, batch=50, learn='adagrad', learning_rate=0.001, regularization=0.0001, samples=0, activation='relu', objective='bpr', epochs=5, last_n_days=None, session_key = 'SessionId', item_key= 'ItemId', time_key= 'Time' ):
...
#                 hit += ( (preds.T > val_pos).sum(axis=0) < 20 ).sum()
    ...
    def init_model(self, train, std=0.01):
        ...
        self.HZ = theano.shared( np.random.normal(0, std, size=(2,self.num_items) ).astype( self.floatX ), name='HZ' )
        self.HR = theano.shared( np.random.normal(0, std, size=(2,self.num_items) ).astype( self.floatX ), name='HR' )
        self.HO = theano.shared( np.random.normal(0, std, size=(2,self.num_items) ).astype( self.floatX ), name='HO' )
    ...
    def _generate_train_model_batch_function(self):
        ...
        #gru integration
        z = self.sigmoid( self.HZ[0] * predI + self.HZ[1] * self.h_t )
        r = self.sigmoid( self.HR[0] * predI + self.HR[1] * self.h_t )
        h_tmp = self.tanh( self.HO[0] * predI + self.HO[1] * self.h_t * r )
        h_t_new = z * h_tmp + (1-z) * self.h_t
        ...
        updates = getattr(self, self.learn)(cost, [self.S,self.I,self.I1,self.I2,self.BI,self.BS,self.HZ,self.HR,self.HO], self.learning_rate)
    ...
    def _generate_predict_function(self):
        ...
        #gru integration
        z = self.sigmoid( self.HZ[0] * predI + self.HZ[1] * self.h_t_p )
        r = self.sigmoid( self.HR[0] * predI + self.HR[1] * self.h_t_p )
        h_tmp = self.tanh( self.HO[0] * predI + self.HO[1] * self.h_t_p * r )
        h_t_p_new = z * h_tmp + (1-z) * self.h_t_p
        
        print( h_t_p_new.shape )
        print( self.h_t_p.shape )
    ...
    def bpr_old(self, predy, y ):
        ...
        obj = T.sum( ( T.log( T.nnet.sigmoid( ytrue - predy ) ) 
                        ...
                        - self.regularization * (self.HZ.T[y] ** 2).sum(axis=1)
                        - self.regularization * (self.HR.T[y] ** 2).sum(axis=1)
                        - self.regularization * (self.HO.T[y] ** 2).sum(axis=1) ) ) 
        return -obj
    ...
    def top1(self, predy, y ):
        ytrue = predy.diagonal()
        obj = T.mean( T.nnet.sigmoid( -ytrue + predy ) 
                       - self.regularization * (self.S[y] ** 2).sum(axis=1)
                        - self.regularization * (self.I[y] ** 2).sum(axis=1)
                        - self.regularization * (self.I1[y] ** 2).sum(axis=1)
                        - self.regularization * (self.I2[y] ** 2).sum(axis=1)
                        ...
                        - self.regularization * (self.BS[y] ** 2)
                        - self.regularization * (self.HZ.T[y] ** 2).sum(axis=1)
                        - self.regularization * (self.HU.T[y] ** 2).sum(axis=1)
                        - self.regularization * (self.HO.T[y] ** 2).sum(axis=1) )
        return obj
