...
class SessionMF:
    ...
    def __init__( self, factors=100, steps=5, batch=32, learn='adagrad_sub', learning_rate=0.01, momentum=0.0, regularization=0.5, dropout=0.0, skip=0, samples=2048, activation='linear', objective='bpr_max', epochs=10, shuffle=-1, last_n_days=None, session_key = 'SessionId', item_key= 'ItemId', user_key= 'UserId', time_key= 'Time' ):
    ...
    def fit(self, data, items=None):
        ...
        self.num_items = data[self.item_key].nunique() + 1
        self.num_users = data[self.user_key].nunique()
        self.itemmap = pd.Series( data=np.arange(self.num_items), index=([data[self.item_key].max()+1]+data[self.item_key].unique()) ).astype( self.intX )
        ...
        for j in range( self.epochs ):
            ...
            svec = np.zeros( (self.batch, self.steps) ).astype( self.floatX )
            ...
            while not finished:
                ...
                svec = np.roll(svec, 1, axis=1)
                svec[ np.arange(self.batch) , 0 ] = items
                ...
                if self.samples > 0:
                    ...
                    loss += self.train_model_batch( svec, items, users, np.hstack( [targets, additional] ), itemsl )
                    avg_time += (time.time() - stmp)
                    avg_count += 1
                else:
                    loss +=  self.train_model_batch( svec, items, users, targets, itemsl )
...
#                     print( 'finished {} of {} in epoch {} with loss {} in {}s'.format( count, len(train), j, ( loss / count ), ( time.time() - start ) ) )
    ...
    def init_model(self, train, std=0.01):
        ...
        self.s = theano.shared( np.random.normal(0, std, size=(self.batch, self.num_items) ).astype( self.floatX ), name='s', borrow=True )
        self.s = theano.shared( np.random.normal(0, std, size=(self.num_items,1) ).astype( self.floatX ), name='s', borrow=True )
    ...
    def _generate_train_model_batch_function(self):
        ...
        si = T.matrix('si', dtype=self.floatX)
        ...
        def set_one_at_position(a_location, output_model):
            zeros = T.zeros_like(output_model)
            zeros_subtensor = zeros[a_location[0], a_location[1]]
            return T.set_subtensor(zeros_subtensor, 1)
        
        s, updates = theano.scan(fn=set_one_at_position,
                                      outputs_info=None,
                                      sequences=[si],
                                      non_sequences=s)
    ...
    def _generate_predict_function(self):
        ...
        si = T.vector('si', dtype=self.floatX)
        ...
        sp, updates = theano.scan(fn=set_one_at_position,
                                      outputs_info=None,
                                      sequences=[si],
                                      non_sequences=sp)
                
        se = T.dot( self.S.T, sp.T )
    ...
    def predict_next( self, session_id, input_item_id, predict_for_item_ids, input_user_id=None, skip=False, type='view', timestamp=0 ):
        ...
        l = len( self.session_items )
        items = self.session_items[ (l - self.steps if l > self.steps else 0)::-1 ]
        items += [0 for i in range(self.steps - len(items))]
        
        predictions = self.predict( items, self.itemmap[input_item_id], self.usermap[input_user_id] )
