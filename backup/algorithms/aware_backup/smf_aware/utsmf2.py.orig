...
class SessionMF:
    ...
    def fit(self, data, items=None):
        ...
        self.s = theano.shared( smat, name='s', borrow=True )
        ...
        for j in range( self.epochs ):
            ...
            while not finished:
                ...
                if self.samples > 0:
                    ...
                    loss += self.train_model_batch( items, users, np.hstack( [targets, additional] ), itemsl )
                    avg_time += (time.time() - stmp)
                    avg_count += 1
                else:
                    loss +=  self.train_model_batch( items, users, targets, itemsl )
...
#                     print( 'finished {} of {} in epoch {} with loss {} in {}s'.format( count, len(train), j, ( loss / count ), ( time.time() - start ) ) )
    ...
    def _generate_train_model_batch_function(self):
        ...
        self.train_model_batch = theano.function(inputs=[i, u, y, items], outputs=cost, updates=updates  )
