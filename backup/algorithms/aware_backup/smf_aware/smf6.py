...
class SessionMF:
    ...
    def __init__( self, factors=100, batch=50, sampling=RAND, learn='adam', learning_rate=0.001, regularization=0.0001, activation='linear', objective='bpr', epochs=5, last_n_days=None, session_key = 'SessionId', item_key= 'ItemId', time_key= 'Time' ):
        ...
        for j in range( self.epochs ):
            ...
            while not finished:
                ...
                for i in range(self.batch):
                    ...
                    sci[i] = self.session_map[ self.sessions[ sidx[i] ] ][ spos[i] - 1 ]
                    svec[i][ sci[i] ] = spos[i]
...
#                             rand.append(i)
                ...
                if self.sampling == SessionMF.RAND:
                    ineg = np.random.randint(self.num_items, size=self.batch)
                elif self.sampling == SessionMF.BATCH:
                    ineg = np.copy(ipos)
                    np.random.shuffle( ineg )
                elif self.sampling == SessionMF.SESSION:       
                    tmp = np.copy(ipos)
                    np.random.shuffle( tmp )
                    ineg[rand] = tmp[rand]
                
                loss += self.train_model_batch( smat, sci, ipos, ineg )
                count += self.batch
...
#                         sneg[i] = list(candidates - to_come)
    ...
    def init_model(self, train, std=0.01):
        ...
        self.BS = theano.shared( np.random.normal(0, std, size=(self.num_items) ).astype( self.floatX ), name='B' )
        self.BI = theano.shared( np.random.normal(0, std, size=(self.num_items) ).astype( self.floatX ), name='B' )
    ...
    def _generate_train_model_batch_function(self):
        ...
        h_t_new =  ( predS + predI ) + ( self.h_t * self.H )
        
        predy = ( h_t_new.T[y] ).diagonal()
        predn = ( h_t_new.T[n] ).diagonal()
        ...
        self.train_model_batch = theano.function(inputs=[s, i, y, n], outputs=cost, updates=updates, on_unused_input='ignore' )
    ...
    def _generate_predict_function(self):
        ...
        h_t_p_new = (predS + predI) + ( self.h_t_p * self.H )
        
        pred = getattr(self, self.activation )( h_t_p_new )
    ...
    def _generate_predict_batch_function(self):
        ...
        pred = getattr(self, self.activation )( h_t_new )
    ...
    def bpr_old(self, predy, predn, y, n ):
        obj = T.sum( ( T.log( T.nnet.sigmoid( predy - predn ) ) 
                        ...
                        - self.regularization * (self.I1[n] ** 2).sum(axis=1)
                        - self.regularization * (self.I2[n] ** 2).sum(axis=1)
                        - self.regularization * (self.BI[y] ** 2)
                        - self.regularization * (self.BI[n] ** 2)
                        - self.regularization * (self.BS[y] ** 2)
                        - self.regularization * (self.BS[n] ** 2)
                        - self.regularization * (self.H[y] ** 2)
                        - self.regularization * (self.H[n] ** 2) ) ) 
        return -obj
