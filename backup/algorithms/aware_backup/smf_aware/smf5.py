...
class SessionMF:
    ...
    def _generate_train_model_batch_function(self):
        ...
        h_t_new =  T.dot( self.I, se ) + ( self.h_t * self.H ).T
        
        predy = ( h_t_new[y] ).diagonal() + self.B[y]
        predn = ( h_t_new[n] ).diagonal() + self.B[n]
        ...
        updates.append( (self.h_t, h_t_new.T) )
    ...
    def _generate_predict_function(self):
        ...
        h_t_p_new = T.dot( self.I, se )  + ( self.h_t_p * self.H ).T
        pred = h_t_p_new + self.B
        
        pred = getattr(self, self.activation )( pred )
        
        self.predict = theano.function(inputs=[s], outputs=pred, updates=[(self.h_t_p, h_t_p_new.T)] )
    ...
    def _generate_predict_batch_function(self):
        ...
        se = T.dot( self.S.T, s.T )
        h_t_new =  T.dot( self.I, se ).T + ( self.h_t * self.H )
        pred = h_t_new + self.B
