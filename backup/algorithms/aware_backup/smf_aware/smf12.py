    ...
    def fit(self, data, items=None):
        ...
        for j in range( self.epochs ):
            ...
            while not finished:
                ...
                if self.samples > 0:
                    additional = np.random.randint(self.num_items, size=self.samples)
...
#                 hit += ( (preds.T > val_pos).sum(axis=0) < 20 ).sum()
    ...
    def init_model(self, train, std=0.01):
        ...
        self.H1 = theano.shared( np.random.normal(0, std, size=(self.num_items) ).astype( self.floatX ), name='H__1' )
        self.h_t1 = theano.shared( np.zeros( ( self.batch, self.num_items ) ).astype( self.floatX ), name='h_t_1' )
        self.h_t_p1 = theano.shared( np.zeros( self.num_items ).astype( self.floatX ), name='h_t_p_1' )
        
        self.H2 = theano.shared( np.random.normal(0, std, size=(self.num_items) ).astype( self.floatX ), name='H__2' )
        self.h_t2 = theano.shared( np.zeros( ( self.batch, self.num_items ) ).astype( self.floatX ), name='h_t_2' )
        self.h_t_p2 = theano.shared( np.zeros( self.num_items ).astype( self.floatX ), name='h_t_p_2' )
    ...
    def _generate_set_vector_function(self):
        ...
        h_t_new1 = T.set_subtensor( self.h_t1[i], s ) 
        h_t_new2 = T.set_subtensor( self.h_t2[i], s ) 
        
        self.set_hidden = theano.function(inputs=[s, i], updates=[(self.h_t1, h_t_new1),(self.h_t2, h_t_new2)])
    ...
    def _generate_train_model_batch_function(self):
        ...
        h_t_1_new = predI + ( self.h_t1 * self.H1 ) + ( self.h_t2 * self.H2 )
        h_t_2_new = self.h_t1

        pred = h_t_1_new + predS
        ...
        updates = getattr(self, self.learn)(cost, [self.S,self.I,self.I1,self.I2,self.BI,self.BS,self.H1,self.H2], self.learning_rate)
        updates.append( (self.h_t1, h_t_1_new) )
        updates.append( (self.h_t2, h_t_2_new) )
    ...
    def _generate_predict_function(self):
        ...
        h_t_p_1_new = predI + ( self.h_t_p1 * self.H1 ) + ( self.h_t_p2 * self.H2 )
        h_t_p_2_new = self.h_t_p1
        
        pred = predS + h_t_p_1_new
        pred = getattr(self, self.activation )( pred )
        
        updates = []
        updates.append( (self.h_t_p1, h_t_p_1_new) )
        updates.append( (self.h_t_p2, h_t_p_2_new) )
    ...
    def _generate_predict_batch_function(self):
        ...
        h_t_new = predI + ( self.h_t1 * self.H1 ) + ( self.h_t2 * self.H2 )
    ...
    def predict_next( self, session_id, input_item_id, predict_for_item_ids, skip=False, type='view', timestamp=0 ):
        ...
        if( self.session != session_id ): #new session      
            ...
            self.session_count = 0
            self.h_t_p1.set_value( np.zeros( (self.num_items) ).astype( self.floatX ) , borrow=True)
            self.h_t_p2.set_value( np.zeros( (self.num_items) ).astype( self.floatX ) , borrow=True)
