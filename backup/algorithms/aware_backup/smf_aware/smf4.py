        ...
        for j in range( self.epochs ):
            ...
            while not finished:
                ...
                if self.samples > 0:
                    ...
                    loss += self.train_model_batch( smat, np.hstack( [ipos, additional] ) )
                else:
...
#                 hit += ( (preds.T > val_pos).sum(axis=0) < 20 ).sum()
    ...
    def _generate_train_model_batch_function(self):
        ...
        h_t_new = predS
        h_t_new = ( h_t_new ) + ( self.h_t * self.H )
        pred = h_t_new
        ...
        updates = getattr(self, self.learn)(cost, [self.S,self.I,self.BS,self.H], self.learning_rate)
        updates.append( (self.h_t, h_t_new) )
        
        self.train_model_batch = theano.function(inputs=[s, y], outputs=cost, updates=updates )
    ...
    def _generate_predict_function(self):
        ...
        h_t_p_new = predS
        h_t_p_new = h_t_p_new + ( self.h_t_p * self.H )
        pred = h_t_p_new
        
        pred = getattr(self, self.activation )( pred )
        
        self.predict = theano.function(inputs=[s], outputs=pred, updates=[(self.h_t_p, h_t_p_new)] )
