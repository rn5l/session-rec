...
class UVMContextKNN:
    '''
    UVMContextKNN( k, sample_size=1000, sampling='recent', similarity='cosine', weighting='div', dwelling_time=False, last_n_days=None, last_n_clicks=None, extend=False, weighting_score='div_score', weighting_time=False, normalize=True, session_key = 'SessionId', item_key= 'ItemId', time_key= 'Time')
    ...
    def __init__( self, k, sample_size=1000, sampling='recent', similarity='cosine', weighting='div', boost_own_sessions=None, extend_user_session=5, dwelling_time=False, last_n_days=None, last_n_clicks=None, extend=False, weighting_score='div_score', weighting_time=False, normalize=True, session_key = 'SessionId', user_key = 'UserId', item_key= 'ItemId', time_key= 'Time' ):
        ...
        self.extend_user_session = extend_user_session
        ...
        self.session_user_map = dict() 
        self.item_session_map = dict()
        self.session_time = dict()
        self.min_time = -1
        
        self.sim_time = 0
        self.boost_own_count = 0
        self.boost_own_count_all = 0
        ...
        for row in train.itertuples(index=False):
            ...
            if row[index_session] != session:
                if len(session_items) > 0:
                    ...
                    self.session_user_map.update({session : row[index_user]})
            ...
            if self.extend_user_session is not None:
                ...
                if len( self.last_user_items[row[index_user]] ) > self.extend_user_session:
                    self.last_user_items[row[index_user]] = self.last_user_items[row[index_user]][-self.extend_user_session:]
...
#         print( 'cknn.predict_next: ', process.memory_info().rss, ' memory used')
        ...
        ext = self.extend_user_session
        if ext is not None and len(items) < ext and input_user_id in self.last_user_items: #update the session with items from the users past
            add = len(items)-ext
            prev_items = self.last_user_items[input_user_id][(-len(items)-add):-add]
        ...
        if not input_user_id in self.last_user_items:
            self.last_user_items[input_user_id] = []
        self.last_user_items[input_user_id].append( input_item_id )
    ...
    def possible_neighbor_sessions(self, session_items, input_item_id, session_id, extending=False):
        '''
    ...
    def calc_similarity(self, session_items, sessions, dwelling_times, timestamp, user_id ):
        ...
        for session in sessions:
            ...
            if similarity > 0:
                ...
                if self.boost_own_sessions is not None and self.boost_own_sessions > 0.0 and self.session_user_map[ session ] == user_id: 
                    similarity = similarity + (similarity*self.boost_own_sessions)
                    self.boost_own_count += 1
                    
                self.boost_own_count_all += 1
    ...
    def find_neighbors( self, session_items, input_item_id, session_id, dwelling_times, timestamp, user_id ):
        ...
        possible_neighbors = self.calc_similarity( session_items, possible_neighbors, dwelling_times, timestamp, user_id )
