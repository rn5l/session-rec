...
class UGRU4Rec:
    ...
    def __init__(self, loss='bpr-max', final_act='linear', hidden_act='tanh', layers=[100],
                 ...
                 adapt='adagrad', adapt_params=[], grad_cap=0.0, bpreg=1.0,
                 sigma=0.0, init_as_normal=False, train_random_order=False, time_sort=True,
                 extend_session_length=None, reminders=False, remind_strategy='recency', remind_sessions_num=6, reminders_num=3, remind_mode='end',
...
#            updates[acc] = T.set_subtensor(acc_s, acc_new) #Faster, but inaccurate when an index occurs multiple times
    ...
    def fit(self, data, test=None, sample_store=10000000):
        ...
        index_user = data.columns.get_loc(self.user_key)
        ...
        if self.hasReminders:  # user_based
            self.reminder.reminders_fit(data, self.user_key, self.item_key, self.time_key)

    def predict_next_batch(self, session_ids, input_item_ids, input_user_id, predict_for_item_ids=None, batch=100):
    ...
    def predict_next(self, session_id, input_item_id, input_user_id, predict_for_item_ids=None, skip=False, mode_type='view', timestamp=0):
        ...
        series = self.predict_next_batch(np.array([session_id]), np.array([input_item_id]), input_user_id, predict_for_item_ids, 1)[0]
    ...
    def extend_session_model_in_loop(self, row, index_user, index_item):
        if not row[index_user] in self.last_user_items:
            self.last_user_items[
                row[index_user]] = []  # create a new list to save the user's last viewed items
    ...
    def predict_for_extended_model(self, input_user_id): # will be called in "evaluation_user_based" and "evaluation_user_based_multiple"
        prev_items = []
