...
class ReminderNew:
    ...
    def __init__(self, remind_strategy='recency', remind_sessions_num=6, reminders_num=3, remind_mode='end', cut_off_threshold = 20):
        ...
        self.users_sessions_items_map = dict()
        self.session_items_map = dict()

    def reminders_fit_in_loop(self, row, index_user, index_session, index_item):
        # create {user_id: [session_id: [item_id, ...] ], ...}
        if not row[index_user] in self.users_sessions_items_map:
            # add user_id to the map
            self.users_sessions_items_map[row[index_user]] = []  # list, because the order is important
        if row[index_session] != self.prev_session_id:  # Just once the new session starts
            self.session_items_map[row[index_session]] = []  # list, because the order is important
            self.session_items_map[row[index_session]].append(row[index_item])  # add first item of the session
            self.users_sessions_items_map[row[index_user]].append(self.session_items_map[row[index_session]])
            # just keep last N sessions
            if len(self.users_sessions_items_map[row[index_user]]) > self.remind_sessions_num:
                del self.users_sessions_items_map[row[index_user]][0]  # delete first session in the list
            # to do not need to add this session id again!
            self.prev_session_id = row[index_session]
        else:
            self.session_items_map[row[index_session]].append(row[index_item])  # add item
        pass
