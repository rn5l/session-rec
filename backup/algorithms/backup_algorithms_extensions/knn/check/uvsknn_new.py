...
from algorithms.extensions.backup.reminder_new import ReminderNew
...
class UVMContextKNN:
    ...
    def __init__(self, k, sample_size=1000, sampling='recent', similarity='cosine', weighting='div',
                 ...
                 reminders_new=False, viewed_before=False, views_count_weight=None, distance_lastView_session_weight=None, distance_lastView_day_weight=None,
        ...
        # SARA
        self.hasRemindersNew = reminders_new
        if self.hasRemindersNew:
            self.reminders_new = ReminderNew(remind_strategy, remind_sessions_num, reminders_num, remind_mode)
            self.viewed_before = viewed_before
            if self.viewed_before:
                self.views_count_weight = views_count_weight
                self.distance_lastView_session_weight = distance_lastView_session_weight
                self.distance_lastView_day_weight = distance_lastView_day_weight
    ...
    def predict_next(self, session_id, input_item_id, input_user_id, predict_for_item_ids=None, skip=False,
        ...
        if self.viewed_before:
            for index, score in series.items():
                print(index)
                print(score)
            for past_session in self.reminder.recent_user_sessions[input_user_id]:  # self.reminder.user_item_recency.loc[input_user_id] # self.last_user_items[input_user_id]
                past_items = self.session_item_map[past_session]  # problem: we only have 1 occurrence of each item
                print(past_items);
            if self.views_count_weight is not None:
                print("weight!")
                # TODO: increase the baseline score
            if self.distance_lastView_session_weight is not None:
                print("weight!")
                # TODO: increase the baseline score
            if self.distance_lastView_day_weight is not None:
                print("weight!")
                # TODO: increase the baseline score
