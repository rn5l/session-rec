...
class GRU4Rec:
    ...
    def init_params(self):
        ...
        pass
    def init_params_retrain(self, n_items_new):
        # if self.constrained_embedding:
        #     n_features = self.layers[-1]
        # elif self.embedding:
        #     self.E.set_value(np.vstack((self.E.get_value(), self.init_weights((n_items_new, self.embedding)).get_value())))
        #     n_features = self.embedding
        # else:
        #     n_features = n_items_new
        # m = []
        # m.append(self.init_matrix((n_features, self.layers[0])))
        # m.append(self.init_matrix((n_features, self.layers[0])))
        # m.append(self.init_matrix((n_features, self.layers[0])))
        # self.Wx[0].set_value(np.vstack((self.Wx[0].get_value(), np.hstack(m))))
        # self.Wy.set_value(np.vstack((self.Wy.get_value(), self.init_weights((n_items_new, self.layers[-1])).get_value())))
        # self.By.set_value(np.vstack((self.By.get_value(), np.zeros((n_items_new, 1)))))
        # self.n_items_prev = n_items_new  # for next round
...
#            updates[acc] = T.set_subtensor(acc_s, acc_new) #Faster, but inaccurate when an index occurs multiple times
    ...
    def model(self, X, H, M, R=None, Y=None, drop_p_hidden=0.0, drop_p_embed=0.0, predict=False):
        ...
        if Y is not None:
            ...
            return H_new, y, sparams, full_params, sidxs
    def generate_neg_samples(self, pop, length): #todo: tasiri nadare k faqat shamele dadahaye feli bashe, na qabli?
    ...
    def fit(self, data, test=None, sample_store=10000000):
        ...
        self.itemidmap = pd.Series(data=np.arange(self.n_items), index=itemids) #todo: naqshe in chie? tasiri nadare k faqat shamele dadahaye feli bashe, na qabli?
        data = pd.merge(data, pd.DataFrame({self.item_key:itemids, 'ItemIdx':self.itemidmap[itemids].values}), on=self.item_key, how='inner')
        offset_sessions = self.init_data(data)
        if self.retrain_mode:
            mask = np.isin(itemids, self.itemids_prev, invert=True)
            items_new = itemids[mask]
            n_items_new = len(items_new)
            self.init_params_retrain(n_items_new)
        else:
            self.init_params()
        self.retrain_mode = True #for next round
        self.itemids_prev = data[self.item_key].unique() #for next round
        if self.n_sample:
