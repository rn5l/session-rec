...
SLICES_NUM = 5
SLICE_INTERVAL = 30  # total_interval = 152 30
DAYS_OFFSET = 0
SAMPLE = False
...
def sample(data, percentage):
    ...
    random.seed(10)
    users=random.sample(users, int(len(users) * percentage / 100))
...
def load_data(file, sample_percentage=None):
    ...
    if not sample_percentage is None:
        print("sample data: "+str(sample_percentage)+"% of users\n")
...
def split_data_slice(data, output_file, slice_id, days_offset, days_shift, min_session_length):
    ...
    # inters = list(greater_start.intersection(lower_end))
    # data = data[data[SESSION_KEY].isin(inters)]
