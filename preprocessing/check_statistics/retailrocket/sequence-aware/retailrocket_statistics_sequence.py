...
def slice_data(data, num_slices=SLICES_NUM, days_offset=DAYS_OFFSET, days_shift=SLICE_INTERVAL, min_session_length=MIN_SESSION_LENGTH):
    for slice_id in range(0, num_slices):
        print("slice " + str(num_slices))
        split_data_slice(data, slice_id, days_offset, days_shift, min_session_length)


def split_data_slice(data, slice_id, days_offset, days_shift, min_session_length):
    ...
    # training-test split
    split_data(data, min_session_length, slice_id)


def last_session_out_interaction(data, min_session_length):
    """
    ...
    idx = data.index
    data['idx'] = idx
    interactions = data.sort_values(by=[USER_KEY, TIME_KEY]).groupby(USER_KEY)
    last_interaction_idx = interactions.last()['idx']
    train = data[~data['idx'].isin(last_interaction_idx.values)].copy()
    test = data[data['idx'].isin(last_interaction_idx.values)].copy()
    if CLEAN_TEST:
...
def split_data(data, min_session_length, slice_id= None): #TODO: extend for supproting more than one sessions per use for test
    """
        assign the last session of every user to the test set and the remaining ones to the training set
        """
    train_full_sessions, test_sessions = last_session_out_interaction(data, min_session_length)
    train_valid_sessions, valid_sessions = last_session_out_interaction(train_full_sessions, min_session_length)
