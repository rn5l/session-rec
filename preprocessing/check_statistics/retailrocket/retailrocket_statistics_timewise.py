...
TRAINING_RATIO = 0.8 # training: 80%, test: 20%
DAYS_INTERVAL = 135
...
def last_days_out_split(data, days_interval, training_ratio=TRAINING_RATIO):
    """
    20% last-days as test data
    80% fisrt-days as training data
    """

    split_day = days_interval * training_ratio
    split = datetime.fromtimestamp(data[TIME_KEY].min(), timezone.utc) + timedelta(split_day)
    #
    # prefilter the timespan
    session_max_times = (data.groupby([SESSION_KEY])[TIME_KEY]).max()
    split_start = session_max_times[session_max_times > split.timestamp()].index
    test = data[np.in1d(data[SESSION_KEY], split_start)]
    train = data[~np.in1d(data[SESSION_KEY], split_start)]

    # data.sort_values(by=[TIME_KEY], ascending=True, inplace=True)
    # data_list = np.array_split(data, PART_NUM)
    # train = pd.DataFrame()
    # for idx in range(0, PART_NUM-1):
    #     train = train.append(data_list[idx], ignore_index=True)
    # test = pd.DataFrame(data_list[PART_NUM-1])
    return train, test

def clean_split_training(train, min_session_length, days_interval, training_ratio=TRAINING_RATIO):
    if CLEAN_TEST:
        # remove items in test that are nor appeared in training set
        ...
        # remove users in test that are nor appeared in training set
...
def clean_split(train, test, min_session_length, days_interval, training_ratio=TRAINING_RATIO):
    if CLEAN_TEST:
        # remove items in test that are nor appeared in training set
        train_items = train[ITEM_KEY].unique()
        test = test[test[ITEM_KEY].isin(train_items)]
...
def split_data(data, min_session_length): #TODO: extend for supproting more than one sessions per use for test
    """
        ...
        """
    train_full_sessions, test_sessions = last_days_out_split(data, days_interval=DAYS_INTERVAL)
    train_valid_sessions, valid_sessions = last_days_out_split(train_full_sessions, days_interval=DAYS_INTERVAL*TRAINING_RATIO)
    filter_data(data)
