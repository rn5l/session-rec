...
PREPARED_FILE = PATH + FILE + '_prepared.txt'
...
def map_user_and_item_id(data):
    ...
    for index, row in data.iterrows():
        ...
        data.at[index, ITEM_KEY] = item_map[item_id]
...
if __name__ == '__main__':
    # data = pd.read_csv(PREPARED_FILE, sep='\t', names=[USER_KEY, TIME_KEY, "artist_id", "artist", ITEM_KEY, "track"])   # Items are tracks
    ...
    data.to_csv(PREPARED_FILE , sep='\t', index=False)
